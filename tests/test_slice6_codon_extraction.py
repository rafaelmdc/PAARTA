...
from lib.codon_extract import extract_call_codons
from lib.repeat_features import CALL_FIELDNAMES, build_call_row, validate_call_row
    ...
    def test_extract_repeat_codons_cli_enriches_successful_rows_and_warns_on_failures(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    str(REPO_ROOT / "bin" / "extract_repeat_codons.py"),
                    "--calls-tsv",
                    str(calls_tsv),
                    "--sequences-tsv",
                    str(sequences_tsv),
