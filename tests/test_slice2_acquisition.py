    ...
    def test_download_normalize_translate_and_merge(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._run_cli(
                [
                    sys.executable,
                    str(REPO_ROOT / "bin" / "download_ncbi_packages.py"),
                    "--batch-manifest",
            ...
            self._run_cli(
                [
                    sys.executable,
                    str(REPO_ROOT / "bin" / "normalize_cds.py"),
                    "--package-dir",
            ...
            self._run_cli(
                [
                    sys.executable,
                    str(REPO_ROOT / "bin" / "translate_cds.py"),
                    "--sequences-tsv",
            ...
            self.assertEqual(len(batch_one_taxonomy), 1)
            self.assertEqual(batch_one_taxonomy[0]["taxon_name"], "Homo sapiens")
            self.assertEqual(batch_one_taxonomy[0]["parent_taxon_id"], "9605")
            self.assertEqual(batch_one_taxonomy[0]["rank"], "species")
            self.assertEqual(batch_one_taxonomy[0]["lineage"], "root > Homo > Homo sapiens")
            self.assertEqual(batch_one_taxonomy[0]["source"], "taxon_weaver:2026-04-05")
            ...
            self._run_cli(
                [
                    sys.executable,
                    str(REPO_ROOT / "bin" / "merge_acquisition_batches.py"),
                    "--batch-inputs",
                    str(batch_one_norm),
                    "--batch-inputs",
                    str(batch_two_norm),
            ...
            self.assertEqual(len(merged_taxonomy), 2)
