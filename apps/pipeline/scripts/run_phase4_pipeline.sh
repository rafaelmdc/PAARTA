...
RUN_ROOT="${HOMOREPEAT_PHASE4_RUN_ROOT:-$REPO_ROOT/runs/$RUN_ID}"
...
DEFAULT_TAXONOMY_DB="$REPO_ROOT/runtime/cache/taxonomy/ncbi_taxonomy.sqlite"
DEFAULT_NXF_HOME="$REPO_ROOT/runtime/cache/nextflow"
...
require_python_package() {
  "$PYTHON_BIN" -c "import homorepeat" >/dev/null 2>&1 || fail "homorepeat is not installed for $PYTHON_BIN; run: $PYTHON_BIN -m pip install -e \"$REPO_ROOT\""
}
...
if [[ "$ACCESSIONS_FILE" != /* ]]; then
  ACCESSIONS_FILE="${REPO_ROOT}/${ACCESSIONS_FILE}"
fi

if [[ -n "$PARAMS_FILE" ]] && [[ "$PARAMS_FILE" != /* ]]; then
  PARAMS_FILE="${REPO_ROOT}/${PARAMS_FILE}"
fi

TAXONOMY_DB="${HOMOREPEAT_TAXONOMY_DB:-$DEFAULT_TAXONOMY_DB}"
if [[ "$TAXONOMY_DB" != /* ]]; then
  TAXONOMY_DB="${REPO_ROOT}/${TAXONOMY_DB}"
fi
...
if [[ $STATUS -eq 0 ]]; then
  ln -sfn "$RUN_ID" "$REPO_ROOT/runs/latest"
fi
