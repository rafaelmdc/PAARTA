...
RUN_ROOT="${HOMOREPEAT_DETECTION_SMOKE_RUN_ROOT:-$REPO_ROOT/runs/$RUN_ID}"
...
find_latest_live_smoke_root() {
  local candidate
  candidate="$(find "$REPO_ROOT/runs" -maxdepth 1 -mindepth 1 -type d -name 'live_smoke_*' | sort | tail -n1 || true)"
  printf '%s' "$candidate"
}
...
if [[ -z "$SOURCE_ACQUISITION_DIR" ]]; then
  [[ -n "$SOURCE_RUN_ROOT" ]] || {
    echo "smoke failure: no source acquisition run was provided and no live_smoke_* run was found under $REPO_ROOT/runs" >&2
    exit 1
  }
