(() => {
  ...
  function showInternalLabels(showLabels, visibleLeafCount) {
    return Boolean(showLabels) && visibleLeafCount <= MAX_VISIBLE_ROWS_WITH_INTERNAL_LABELS;
  }
  ...
  function layoutWidths(payload, options) {
    ...
    const showSplitLabels = showInternalLabels(showLabels, visibleLeafCount);
  ...
  function buildGraphics(chart, payload, state) {
    const graphics = [];
  ...
  function attach(chart, { payload }) {
    ...
    function render(options = {}) {
      ...
      chart.setOption(
        { graphic: buildGraphics(chart, payload, state) },
        { replaceMerge: ["graphic"], lazyUpdate: true },
      );
    }
